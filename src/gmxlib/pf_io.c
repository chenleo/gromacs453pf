...
gmx_large_int_t pf_atoms_summed_to_fm(t_pf_atoms *atoms, int **fmatoms) {
  t_pf_atom_summed i_atom_summed;
  gmx_large_int_t interactions_count;
  int *fm;
  atom_id i;
...
gmx_large_int_t pf_atoms_scalar_to_fm(t_pf_atoms *atoms, int **fmatoms) {
...
rvec *pf_residues_com(t_pf_global *pf_global, gmx_mtop_t *top_global, const rvec *x) {
  ...
  /* there might be residues with no interesting atoms, so their mass would be set to the initial ZERO;
...
void pf_write_frame_scalar(t_pf_atoms *atoms, FILE* f, int *framenr, const rvec *x) {
  atom_id i, ii, j, jj;
  ...
  switch (pf_global->OnePair) {
    ...
    case PF_ONEPAIR_SUMMED:
      switch (pf_global->AtomBased) {
        ...
        case PF_FILE_OUT_COMPAT_BIN:
        case PF_FILE_OUT_COMPAT_ASCII:
          pf_write_frame_atoms_summed_compat(pf_global->atoms, pf_global->of_atoms, &pf_global->nsteps_atoms, x, (pf_global->AtomBased == PF_FILE_OUT_COMPAT_ASCII), pf_global->Vector2Scalar);
          break;
        case PF_FILE_OUT_PER_ATOM_SUM:
          ...
          break;
        case PF_FILE_OUT_PER_ATOM_AVERAGE:
          pf_per_atom_average(pf_global->per_atom_real_int, pf_global->atoms->summed, pf_global->atoms->len, x, pf_global->Vector2Scalar);
          pf_per_atom_real_write_frame(pf_global->of_atoms, pf_global->per_atom_real_int->force, pf_global->per_atom_real_int->len, pf_global->no_end_zeros);
          break;
        case PF_FILE_OUT_PER_ATOM_MIN:
          pf_per_atom_minmax(pf_global->per_atom_real, pf_global->atoms->summed, pf_global->atoms->len, FALSE, x, pf_global->Vector2Scalar);
          pf_per_atom_real_write_frame(pf_global->of_atoms, pf_global->per_atom_real->force, pf_global->per_atom_real->len, pf_global->no_end_zeros);
          break;
        case PF_FILE_OUT_PER_ATOM_MAX:
          pf_per_atom_minmax(pf_global->per_atom_real, pf_global->atoms->summed, pf_global->atoms->len, TRUE, x, pf_global->Vector2Scalar);
          pf_per_atom_real_write_frame(pf_global->of_atoms, pf_global->per_atom_real->force, pf_global->per_atom_real->len, pf_global->no_end_zeros);
          break;
        default:
          break;
      ...
      switch (pf_global->ResidueBased) {
        ...
        case PF_FILE_OUT_COMPAT_BIN:
        case PF_FILE_OUT_COMPAT_ASCII:
          pf_write_frame_atoms_summed_compat(pf_global->residues, pf_global->of_residues, &pf_global->nsteps_residues, (const rvec*)com, (pf_global->ResidueBased == PF_FILE_OUT_COMPAT_ASCII), pf_global->Vector2Scalar);
          break;
        case PF_FILE_OUT_PER_ATOM_SUM:
          ...
          break;
        case PF_FILE_OUT_PER_ATOM_AVERAGE:
          pf_per_atom_average(pf_global->per_residue_real_int, pf_global->residues->summed, pf_global->residues->len, (const rvec*)com, pf_global->Vector2Scalar);
          pf_per_atom_real_write_frame(pf_global->of_residues, pf_global->per_residue_real_int->force, pf_global->per_residue_real_int->len, pf_global->no_end_zeros);
          break;
        case PF_FILE_OUT_PER_ATOM_MIN:
          pf_per_atom_minmax(pf_global->per_residue_real, pf_global->residues->summed, pf_global->residues->len, FALSE, (const rvec*)com, pf_global->Vector2Scalar);
          pf_per_atom_real_write_frame(pf_global->of_residues, pf_global->per_residue_real->force, pf_global->per_residue_real->len, pf_global->no_end_zeros);
          break;
        case PF_FILE_OUT_PER_ATOM_MAX:
          pf_per_atom_minmax(pf_global->per_residue_real, pf_global->residues->summed, pf_global->residues->len, TRUE, (const rvec*)com, pf_global->Vector2Scalar);
          pf_per_atom_real_write_frame(pf_global->of_residues, pf_global->per_residue_real->force, pf_global->per_residue_real->len, pf_global->no_end_zeros);
          break;
        default:
          break;
...
void pf_write_frame_atoms_scalar_compat(t_pf_atoms *atoms, FILE *f, int *framenr, const rvec *x, gmx_bool ascii) {
  t_pf_atom_scalar i_atom_scalar;
...
void pf_write_scalar_time_averages(t_pf_global *pf_global, const rvec *x) {
  ...
  if (pf_global->AtomBased) {
    ...
    if (pf_in_compatibility_mode(pf_global->AtomBased))
      pf_write_frame_atoms_scalar_compat(pf_global->atoms, pf_global->of_atoms, &pf_global->nsteps_atoms, x, (pf_global->AtomBased == PF_FILE_OUT_COMPAT_ASCII));
    else
      pf_write_frame_scalar(pf_global->atoms, pf_global->of_atoms, &pf_global->nsteps_atoms, x);
  ...
  if (pf_global->ResidueBased) {
    ...
    if (pf_in_compatibility_mode(pf_global->ResidueBased))
      pf_write_frame_atoms_scalar_compat(pf_global->residues, pf_global->of_residues, &pf_global->nsteps_residues, (const rvec*)pf_global->time_averages->com, (pf_global->ResidueBased == PF_FILE_OUT_COMPAT_ASCII));
    else
      pf_write_frame_scalar(pf_global->residues, pf_global->of_residues, &pf_global->nsteps_residues, (const rvec*)pf_global->time_averages->com);
...
void pf_save_and_write_scalar_time_averages(t_pf_global *pf_global, const rvec *x,  gmx_mtop_t *top_global) {
  ...
  if ((pf_global->time_averages->period != 0) && (pf_global->time_averages->steps >= pf_global->time_averages->period))
    pf_write_scalar_time_averages(pf_global, x);
}
