...
const char *pf_file_out_atom_option[PF_FILE_OUT_NR+1] = {
  "no",
  "scalar",
  "vector",
  "compat_bin",
...
const char *pf_file_out_residue_option[PF_FILE_OUT_NR+1] = {
  "no",
  "scalar",
  "vector",
  "compat_bin",
...
void pf_output_data_per_frame_init(t_pf_global *pf_global) {
  ...
  switch (pf_global->ResidueBased) {
    case PF_FILE_OUT_PER_ATOM_MIN:
      pf_per_atom_real_set(pf_global->per_residue_real, GMX_REAL_MAX);
      break;
    case PF_FILE_OUT_PER_ATOM_MAX:
      pf_per_atom_real_set(pf_global->per_residue_real, -GMX_REAL_MAX);
      break;
...
t_pf_global *pf_init(FILE *fplog, int nfile, const t_filenm fnm[], gmx_mtop_t *top_global) {
  ...
  inp = read_inpfile(pf_file_in, &ninp, NULL, wi);
/*  {
  int i;
  for (i=0; i < ninp; i++) {
...
*/  
  ...
  STYPE("pf_onepair", tmpstr, "summed");
  ...
  STYPE("pf_group1", g1name, "Protein");
  STYPE("pf_group2", g2name, "Protein");
  ...
  STYPE("pf_atombased", tmpstr, "no");
  pf_global->AtomBased = PF_FILE_OUT_NONE;
  for (i = 0; i < PF_FILE_OUT_NR; i++)
    if (strcasecmp(tmpstr, pf_file_out_atom_option[i]) == 0)
      pf_global->AtomBased = i;
  
  STYPE("pf_residuebased", tmpstr, "no");
  pf_global->ResidueBased = PF_FILE_OUT_NONE;
  for (i = 0; i < PF_FILE_OUT_NR; i++)
    if (strcasecmp(tmpstr, pf_file_out_residue_option[i]) == 0)
      pf_global->ResidueBased = i;

  fprintf(stderr, "AtomBased: %s, ResidueBased: %s\n", pf_file_out_atom_option[pf_global->AtomBased], pf_file_out_residue_option[pf_global->ResidueBased]);
  ...
  if (((pf_file_out_per_atom(pf_global->AtomBased)) || (pf_file_out_per_atom(pf_global->ResidueBased))) && (pf_global->OnePair != PF_ONEPAIR_SUMMED))
   gmx_fatal(FARGS, "Per atom data can only be computed from summed interactions.\n");

  STYPE("pf_residuesrenumber", tmpstr, "auto");
  ...
  if (pf_global->AtomBased) {
    ...
    switch (pf_global->AtomBased) {
      case PF_FILE_OUT_PER_ATOM_SUM:
        pf_per_atom_real_init(&(pf_global->per_atom_real), pf_global->syslen_atoms, ZERO);
        break;
      case PF_FILE_OUT_PER_ATOM_AVERAGE:
        pf_per_atom_real_int_init(&(pf_global->per_atom_real_int), pf_global->syslen_atoms, ZERO, 0);
        break;
      case PF_FILE_OUT_PER_ATOM_MIN:
        pf_per_atom_real_init(&(pf_global->per_atom_real), pf_global->syslen_atoms, GMX_REAL_MAX);
        break;
      case PF_FILE_OUT_PER_ATOM_MAX:
        pf_per_atom_real_init(&(pf_global->per_atom_real), pf_global->syslen_atoms, -GMX_REAL_MAX);
        break;
    ...
    if (!pf_global->ofn_atoms)
      gmx_fatal(FARGS, "No file (-pfa) for writing out atom-based pairwise forces.\n");
  }
  if (pf_global->ResidueBased) {
    pf_check_sys_in_g(pf_global);
    ...
    switch (pf_global->ResidueBased) {
      case PF_FILE_OUT_PER_ATOM_SUM:
        pf_per_atom_real_init(&(pf_global->per_residue_real), pf_global->syslen_residues, ZERO);
        break;
      case PF_FILE_OUT_PER_ATOM_AVERAGE:
        pf_per_atom_real_int_init(&(pf_global->per_residue_real_int), pf_global->syslen_residues, ZERO, 0);
        break;
      case PF_FILE_OUT_PER_ATOM_MIN:
        pf_per_atom_real_init(&(pf_global->per_residue_real), pf_global->syslen_residues, GMX_REAL_MAX);
        break;
      case PF_FILE_OUT_PER_ATOM_MAX:
        pf_per_atom_real_init(&(pf_global->per_residue_real), pf_global->syslen_residues, -GMX_REAL_MAX);
        break;
    ...
    if (!pf_global->ofn_residues)
      gmx_fatal(FARGS, "No file (-pfr) for writing out residue-based pairwise forces.\n");
  }

  STYPE("pf_type", tmpstr, "all");
  ...
  STYPE("pf_vector2scalar", tmpstr, "norm");
  ...
  if (((pf_in_compatibility_mode(pf_global->AtomBased)) || (pf_in_compatibility_mode(pf_global->ResidueBased))) && (pf_global->Vector2Scalar != PF_VECTOR2SCALAR_NORM))
  ...
  if (pf_global->time_averages->period != 1) {
    ...
    if (pf_global->AtomBased) {
      if (!((pf_in_compatibility_mode(pf_global->AtomBased)) || (pf_global->AtomBased == PF_FILE_OUT_SCALAR)))
    ...
    if (pf_global->ResidueBased) {
      if (!((pf_in_compatibility_mode(pf_global->ResidueBased)) || (pf_global->ResidueBased == PF_FILE_OUT_SCALAR)))
        gmx_fatal(FARGS, "Can only use time averages with scalar or compatibility output.\n");
  ...
  STYPE("pf_no_end_zeros", tmpstr, "no");
