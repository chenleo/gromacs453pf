 ...
 *  For pairwise forces, a pointer to forcerec was added; this makes the above
 *  assumption void, but pairwise forces are only implemented in C anyway...
 *
...
/** Pairwise forces nonbonded lernels type.
 *
 * Similar to the above type, but with the force record added. This is needed to directly
 * access the pairwise forces data.
 * 
 *  \param fr        Force record.
 */
typedef void
pf_nb_kernel_t(int *             nri,
            int *             iinr,
            int *             jindex,
            int *             jjnr,
            int *             shift,
