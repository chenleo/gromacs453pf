					  ...
					  int *                inneriter)
{
	...
	for(n=0; (n<nlist->nri); n++)
    {
			...
			if(ivdw>0)
			{
				...
				switch(ivdw)
				{
					case 1:
						...
						pf_lj            = (12.0*Vvdw_rep-6.0*Vvdw_disp)*rinvsq;
						fscal           += pf_lj;
					...
					case 2:
						...
						pf_lj            = (br*Vvdw_rep-6.0*Vvdw_disp)*rinvsq;
						fscal           += pf_lj;
						Vvdwtot          = Vvdwtot+Vvdw_rep-Vvdw_disp;
						break;
		...
		if (pf_global->bInitialized)
		  pf_atom_add_nonbonded(fr->pf_global, ii, jnr, pf_coul, pf_lj, dx, dy, dz);
        }
