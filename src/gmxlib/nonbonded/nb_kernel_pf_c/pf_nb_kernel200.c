    ...
    do
    {
        ...
        *count           = nn1;            
		...
		if (pf_global->bInitialized)
		  pf_atom_add_nonbonded_single(pf_global, ii, jnr, PF_INTER_COULOMB, fscal, dx11, dy11, dz11);
...
/*
 * Gromacs nonbonded kernel pf_pf_nb_kernel200nf
 * Coulomb interaction:     Reaction field
 * VdW interaction:         Not calculated
 * water optimization:      No
 * Calculate forces:        no
 */
void pf_nb_kernel200nf(
                    int *           p_nri,
                    int *           iinr,
                    int *           jindex,
                    int *           jjnr,
