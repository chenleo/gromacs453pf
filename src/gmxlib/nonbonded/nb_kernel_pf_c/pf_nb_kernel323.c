    ...
    do
    {
        ...
        *count           = nn1;            
		...
		if (pf_global->bInitialized)
		  pf_atom_add_nonbonded_single(pf_global, ii, jnr, PF_INTER_LJ, fscal, dx11, dy11, dz11);
                /* Load parameters for j atom */
                jq               = charge[jnr+0];  
                qq               = qH*jq;          
		...
		if (pf_global->bInitialized)
		  pf_atom_add_nonbonded_single(pf_global, ii+2, jnr, PF_INTER_COULOMB, fscal, dx31, dy31, dz31);
	        ...
	        /* pairwise forces */
		if (pf_global->bInitialized)
		  pf_atom_add_nonbonded_single(pf_global, ii+3, jnr, PF_INTER_COULOMB, fscal, dx41, dy41, dz41);
...
/*
 * Gromacs nonbonded kernel pf_nb_kernel323nf
 * Coulomb interaction:     Tabulated
 * VdW interaction:         Buckingham
 * water optimization:      TIP4P - other atoms
 * Calculate forces:        no
 */
void pf_nb_kernel323nf(
                    int *           p_nri,
                    int *           iinr,
                    int *           jindex,
                    int *           jjnr,
